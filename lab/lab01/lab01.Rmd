...
### Instructions 
* Due date: Friday, August 26th, 10:00 pm PT with 2 hour grace period.
...
```
```{r five-wrong-again}
# This is a REALLY WRONG answer. Change it back to 5 afterwards to compare the autograder response!
p3 <- NULL # FILL IN WITH STRING
...
Now, at the end of the assignment, you should go back through and run each code chunk to be sure you passed all of your tests. On this assignment, that only means that `p1`, `p2`, and `p3` should all equal 5. Go back and change your answers if they are not! One way to check that works well is to click the grey downward facing arrow in the right corner of the code chunk. This will run all previous code chunks so all of your answers are up to date.
...
cd; cd ph142-fa22/lab/lab01; python3 turn_in.py
